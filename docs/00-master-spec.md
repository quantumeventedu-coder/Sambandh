...
## F1 — Authentication (email/phone OTP · password · Google · passkeys)

- **Login methods (all live):** **email OTP** (primary, no SMS), **phone OTP**, **username + password** (`POST /auth/register`, `POST /auth/login` — bcrypt-hashed, 2FA-aware, user-enumeration-safe compare), **Google account** (`POST /auth/google` — verifies the Google ID token via `google-auth-library`, gated behind `GOOGLE_CLIENT_ID`, returns 503 until configured; `GET /auth/config` exposes the client id to the login screen), and **WebAuthn passkeys** (F1 below). An account is keyed by any of email / phone / username / googleId (each unique-when-present). A shared `completeLogin()` runs the account gates + 2FA + token for password/Google. Verified live on Supabase (register, login by username/email, wrong-password 401, duplicate 409, Google 503-when-unconfigured).
- **Purpose (OTP):** Passwordless signup/login. **Email is the primary channel** (no SMS); phone is an optional alternative. An account is keyed by whichever identifier is used (both are unique-when-present; at least one is set).
...
- **Auth hierarchy:** `SUPER_ADMIN_KEY` (header `X-Super-Key`) passes every admin route *and* the super admin routes; `ADMIN_API_KEY` passes admin routes only and is rejected by super admin routes. Both keys are supplied via the environment only — never hardcoded. The old committed dev keys are permanently rejected at boot in production (src/config/require-secrets.js).
