...
- **Business logic:**
  ...
  - Selfie: liveness + face-match against ID photo (dev simulation: confidence 0.985; a payload starting with bytes `FAIL` deterministically fails for testing). Approved selfie re-verification required after 24 months (`expiresAt`).
